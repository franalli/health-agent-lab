...
TrajectoryAnalysis {
  ...
  markers: [ {
    ...
    clinical_change: { rcv, exceeds_rcv },   // RCV: did the change clear the marker's own noise?
...
│   ├── health_intelligence/
│   │   ├── __init__.py
│   │   ├── config.py            # model pin · stats policy (α · FDR q · n_min · Theil–Sen CI) · per-marker clinical constants (CVa/CVi · adverse direction · panic thresholds · band cutpoints · Vitamin-D bands · vital bounds) · config_version
