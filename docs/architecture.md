...
No non-numeric/censored lab values (`lab_results.value` is numeric, which the trajectory markers here are — accommodating non-numeric markers is a small additive change only if such markers appear); no RAG; no image or VLM tier (inputs are structured panels + pre-parsed text, so there is no pixel input for a VLM to own — notes arrive already parsed); no trained ML in v1 (rules + statistics — no labels yet, and rules are more auditable for a safety-critical first version); the input gate is LLM-classified in v1 with a deterministic **emergency-phrase floor** beneath it (Pydantic-enum route + message floor + the non-overridable self-harm floor of §98, few-shot on held-out hard-case exemplars) — the broader **injection/jailbreak denylist** and a **learned** intent classifier after it — the classic-ML evolution of the gate, a trained model (sentence embeddings + logistic regression, or a fine-tuned encoder) once production traffic yields a labeled message corpus — are the named seams, deferred because those are production properties a tiny eval set can't validate (a supervised classifier can't be trained on 17 cases without overfitting); no caching layer (data is tiny, recompute is instant); no job queue (the one multi-minute operation, `/learn`, runs synchronously — acceptable because it is single-flight-locked and `feedback`-set-debounced now that the handover surface exposes it, so those guards replace the operator-only assumption; a background-job runner is the seam once it runs unattended or at scale); no cross-*instance* concurrency handling (the deploy runs **`--workers 2`** on one host against the one WAL SQLite file, and the multi-WORKER seams are handled: the pure core and atomic idempotent UNIQUE-key writes make every safety guarantee worker-count-independent, §15; WAL mode + a `busy_timeout` on every connection (`db.connect`) give a concurrent reader the last consistent snapshot during a destructive admin op (`/admin/reseed`'s atomic reseed, `/members/upload`) instead of a `SQLITE_BUSY` 500; the active `prompt_version` is read from the DB per request (not cached in-process) with a connection per request; and the check-then-act sequences that SPAN statements are serialized by `db.process_lock` — a `fcntl.flock` advisory lock on a file beside the DB, auto-released with its holder. Three lock domains: the BLOCKING **`startup`** lock makes exactly one worker run the lifespan's fresh-DB `executescript` + seed-if-empty + migrations (the loser waits out the sub-second init, then no-ops through every step — without it the loser crashed on "table already exists"/"duplicate column"; the blocking acquire is BOUNDED — a wedged holder raises `TimeoutError` after 60s instead of an untimed flock wait that is SIGTERM-immune, since the handler runs but PEP 475 retries the syscall), the non-blocking **`learn`** LEARNING-STATE lock is shared three ways — `/learn`'s single-flight (layered under its in-process `threading.Lock`, so the multi-minute credit-burning gate can't double-run and race `next_prompt_version`) plus `/reset` and `/admin/reseed`, which **409** rather than truncate/flip the `feedback`/`prompt_versions` tables a mid-flight gate run is reading — and the non-blocking **`data`** DATASET-INGEST lock shared by `/members/upload`, `POST /members`, and `/admin/reseed`, so a multi-transaction bulk ingest can't interleave with the factory reset's truncate + folder-rmtree (which stranded uploaded members inside a "factory" DB whose dataset folder was just deleted, or erased an upload's committed members under its own 200). Only CONTENTION (flock's `BlockingIOError`) is a soft outcome — any other lock `OSError` (ENOLCK, a lock-less filesystem) propagates loudly, since yielding "busy" for a broken locking facility would run startup unserialized and misreport an infra fault as an endless 409. The multi-statement writers themselves ride single transactions: `pipeline.scan` is one `BEGIN IMMEDIATE` transaction (the load/analyze read joins the write atomically, so a stale concurrent scan can no longer supersede a live clinician_review or prune a fresh observation another worker just emitted), the startup seed commits all-or-nothing (a SIGKILL mid-seed leaves the DB empty for a clean retry instead of a stranded partial prefix the emptiness guard reads as 'seeded'), and `/reset` captures its revert re-scan list via the deactivating `UPDATE … RETURNING` (one statement — no read-then-act gap against a concurrent `/feedback`). `db.connect` additionally retries the one-time delete→WAL journal transition, which SQLite refuses to busy-wait on deadlock-avoidance grounds — the one race a fresh DB's double-boot hits *before* any lock can exist. flock's scope is one HOST, which *matches* the topology: the persistent disk pins Render to a single instance, so true multi-INSTANCE coordination remains the named deferred seam, triggered with auth/multi-tenancy); a PARTIAL observation/escalation lifecycle — escalations now carry a `status` (`open` | `superseded`) that the scan's `db.reconcile_escalation_status` sets: a `clinician_review` data-finding whose flag a `/feedback` override CLEARED flips to `superseded`, so it leaves the active `GET /escalations` worklist while its row is KEPT for audit (the per-member `get_escalations` still returns it), and it re-`open`s symmetrically if the override is removed. **Safety-scoped:** an `urgent` (panic) escalation is NEVER auto-superseded — a fired urgent stays on the queue until a human clears it (the deferred human-resolve slice); only the softer `clinician_review` tier auto-clears, keyed on the `live_escalation_ids` emitted-set the scan writes (so a stale `clinician_review` twin a `range_override` displaced is superseded). Accepted consequence: a `range_override` on a *still-urgent* marker leaves the old + new `marker_version` both open — a safe-direction over-show, deliberately NOT de-duplicated (a conditional supersede of the urgent twin was tried and reverted: keyed on emitted obs-ids it could HIDE a *downgraded* urgent — panic→clinician_review — or resurrect a superseded one, both worse than the duplicate). `suppress_marker` is itself inert against a panic floor (`db._latest_breaches_panic`, a pre-existing safety-floor bug fixed alongside this, where a clinician suppress of a panic marker used to drop the floor `urgent→none`). The `acknowledged` half of that HUMAN lifecycle has since LANDED: an active clinician/system-sourced `escalation_accept` feedback row (target = the escalation_id, or its observation_id for a data finding; a member-sourced accept is stored but INERT on the queue — the overlay is source-filtered like `resolve_overrides`, §7's "the member experience cannot lobby the safety logic") renders its escalation `status='acknowledged'` — a READ-TIME overlay over a stored 'open', derived in the shared escalation read (`db._ESCALATION_COLUMNS`) and deliberately NEVER persisted (the stored column stays two-valued, so no CHECK-rebuild migration; `/reset` reverts by deactivating the accept row; the scan's reconcile stays acknowledgment-blind — a re-scan cannot un-acknowledge). An acknowledged escalation STAYS on the queue, visibly triaged (ordered after un-acknowledged within its severity tier — severity still dominates, an acknowledged urgent never sinks below an open clinician_review), and a stored 'superseded' WINS over the overlay; who-acked-it-when is the feedback row itself. The remaining DEFERRED slice is `resolved` — the capability that lets a person take an item (including an urgent) OFF the queue, the last step separating a triage worklist from an append-only log; a fine scope cut for this build, but a deliberate one; no auth/multi-user/persistence beyond the prototype — on the handover the **privileged routes** (`/learn`, `/admin/reseed`, `DELETE /members/{id}`) are exposed on synthetic single-tenant data rather than auth-gated, with `/learn`'s cost guards (debounce + cap + single-flight + pre-check) standing in for access control and `/admin/reseed`'s global wipe acceptable because the demo data is disposable and re-seedable; the same gap shapes the **global `GET /escalations` queue** — it reads escalations across *every* member, correct on synthetic single-tenant data but a cross-panel privacy leak the moment there are real clinicians, so in prod "global" must mean a `clinician_id`/panel scope, not literally all members; auth + per-tenant isolation are the trigger for a real multi-tenant deployment; no decompression-size cap on the upload path (`POST /members/upload` reads each zip entry with no expanded-size ceiling, so a decompression-bomb entry is a theoretical memory-DoS) — deferred as the same scope class as the un-gated privileged routes above (operator-only, synthetic single-tenant) with the same trigger (the untrusted-upload boundary, i.e. when auth lands), the one-line remedy being a cumulative `ZipInfo.file_size` ceiling before `zf.read` in the firewall's `_zip_entries`; no fine-tuning/GPUs; no multilingual. Each is named with the trigger that would make us build it. **Scope priority:** the two core behaviors, both modes (deterministic Mode 1 + LLM Mode 2), the safety/escalation floor, and the eval harness are load-bearing; the live self-improvement slice, hosting, and the LangSmith trace sink are additive layers — the first to be cut under time pressure, not the foundation.
...
| `POST /members/{id}/scan` | Run the proactive scan now — a manual **re-run** of the same idempotent scan every input-changing action already ends in (the ingest paths: seed / reseed / upload; and the override paths: a core `/feedback` correction, `/reset`), kept for demonstrating the mechanism and as the **retry** when a best-effort auto-scan failed; returns the observations raised plus `new_observations` — how many of them this run **newly persisted** (row-identity newness on the deterministic `(member, marker, data_version)` observation id: an idempotent re-scan reports 0; an override re-scan counts exactly the re-raised markers; a genuine data change mints new ids, so re-confirmed findings count as new against the new data — `models.ScanResult`). The operator panel's scan-all sums it, so the readout answers "did this scan find anything new?" at a glance. | → `{observations: Observation[], new_observations}` |
| `POST /members/{id}/feedback` | Record a correction (range_override / suppress_marker / preference) or a signal (helpful / incorrect / escalation_accept / escalation_reject). Corrections feed the deterministic core / runtime composer; a **core** correction (range_override / suppress_marker) **auto-scans the member** after its row commits — best-effort, like every auto-scan path — so the persisted Observations + escalation worklist reflect the override when the call returns (the apply-side twin of `/reset`'s revert-side re-scan). Only a **learn-channel** signal (`incorrect` → few-shot exemplar, *recurring* `escalation_reject` → tone clause) feeds `/learn` — `helpful`/`escalation_accept` are advisory. An `escalation_reject` carries a `{reason}` payload descriptor — *why* the escalation is a false alarm — required by the operator form and deterministically bounded at submission (`learn._screen_reject_reason`: present-but-blank, non-text, or oversized → 422; no judge, since the reason is audit text, never prompt text — the `/learn` tone clause is pre-written; a payload-less programmatic reject stays valid, the toggle counts by kind). | `Feedback` → `{feedback_id, rescanned, acknowledged, channel, effect}` (`learn.classify_feedback`; `acknowledged` True iff a clinician/system-sourced `escalation_accept`'s target matched a live escalation, which the queue now shows as acknowledged — a member-sourced accept is stored but inert on the queue; `channel ∈ core/composer/learn/advisory`, so the UI advertises Run learn only for a `learn` channel — no redundant `learnable` flag; `rescanned` True iff this submission's auto-scan ran cleanly — False for a non-core kind, which changes no analysis input and skips it, or a failed scan, where the manual Scan is the retry) |
| `GET /members/{id}/feedback` | The member's full submitted-feedback trail, newest-first — the read-only **audit projection** over the `feedback` table (corrections and signals alike, each with its payload — e.g. an `escalation_reject`'s `{reason}` descriptor). Includes `active=false` rows: `/reset` deactivates rather than deletes, so the trail survives with the flag showing which rows still feed the live consumers. A pure read that feeds nothing — the live paths (`resolve_overrides` / `get_active_preferences` / `get_active_signals`) consume active rows through their own seams, never through this one. 404 on an unknown member (an empty trail must not read as "no such member"). Surfaced as the operator panel's **Member feedback** button (Clinician group). | — → `list[FeedbackRecord]` (the input `Feedback` + `feedback_id`, `active`, `created_at`) |
...
| `GET /escalations` | **The clinician-review queue** — every live escalation across **all members** — an `escalation_accept`-acknowledged one stays listed with `status='acknowledged'` (the §720 read-time overlay), ordered after un-acknowledged within its tier; only `superseded` leaves the queue — ranked highest-severity first (each item carries a derived `severity` — `urgent` → 'urgent' before `clinician_review` → 'attention') then most-recent. The cross-member triage worklist and *the* hand-off surface (escalation = "make sure a human sees this", which a per-member read can't guarantee). Tenant-wide here (no clinician identity); a `clinician_id`/panel scope in prod (§15). | → `Escalation[]` (each carries `member_id`) |
| `GET /members/{id}/escalations` | Per-member **drill-in** — one member's standing escalation record (highest-severity first like the queue, oldest-first within a tier), shown when already viewing that member. The detail view, **not** the queue (that is `GET /escalations`). | → `Escalation[]` |
| `GET /prompts` | The composer-prompt **version history**, newest first — every `prompt_versions` row with its gate verdict (`status`: `promoted` / `rejected` / `reverted` by `/reset`), an `active` flag on the version the composer serves now (the latest promoted — `db.get_active_prompt`'s pick, derived from the same ordering), the full `prompt_text`, and a compact `eval_summary` digest of the stored gate report (never the full `eval_report_json` — it embeds the raw case set). The read-only audit view of the learning loop: prompts are **learnt from feedback and validated by `/learn`** before they serve. Surfaced as the operator panel's **Prompt history** button (Learning group). | — → `list[PromptVersionRecord]` |
...
| `POST /reset` | Erase learning → v0: deactivate feedback, revert the prompt to baseline; re-scans the affected members (the revert side), reporting `rescanned` + `new_observations` (restored rows re-created by the revert re-scan). | → `{learning_reset, rescanned, new_observations, …counts}` |
| `POST /admin/reseed` | **Factory reset** (demo hygiene; distinct from `/reset`): truncate all tables, restore baseline `prompt_version` v0, re-ingest the `training_data` bundle only — back to the initial 15-member state, uploaded holdouts dropped (DB members via the truncate AND their on-disk dataset FOLDERS via `datasets.remove_uploaded_datasets`, best-effort after commit, so the datasets root is left holding only the shipped bundle) — then **auto-scan** the restored members (every ingest path ends in a scan, so Observations are populated when the call returns; the scan runs AFTER the atomic reseed transaction commits, best-effort per member). An operator op, not a member feature. | → `{reseeded, scanned, new_observations, datasets_removed, …counts}` |
| `GET /health` | Liveness. | → `{status}` |

**Not a route (by design):** the **eval harness** imports the pipeline and runs the labeled set headlessly — the same harness `/learn` calls to gate a prompt candidate (on its critical composer subset, `eval.adapter.LEARN_GATE_CASE_IDS`). The loader CLI (`python -m preprocessing.ingest <bundle>`) stays for local seeding — it too ends in the auto-scan (every ingest path does; it is the `make seed` behind `make run`, so the local one-command boot lands with observations populated); `POST /members` is its live equivalent for a held-out dataset.
...
```
health-intelligence/
├── README.md                    # the instructions manual — hosted-demo usage · operator console · local run · deploy; AI-tools note
├── CLAUDE.md                    # operating manual for AI-agent work in this repo (invariants + commands)
├── render.yaml                  # Render Blueprint (repo root, where the platform reads it) — §15
├── .pre-commit-config.yaml      # ruff + gitleaks + hygiene hooks (repo root: gates the whole tree)
├── .python-version              # pins Python 3.12 (uv locally; Render reads it from the root)
├── backend/
│   ├── Makefile                 # init-db · seed · run · serve · eval · eval-judge · lint · test — run from backend/
│   ├── pyproject.toml
...
├── docs/
│   ├── architecture.md          # this document — design + rationale
│   └── ui-ux.md                 # the member + operator surface spec
└── frontend/
...
**Local run — Python-only, one command.** `git clone`; put `ANTHROPIC_API_KEY` in `backend/.env`; `make run` creates the venv, installs, inits + seeds the SQLite DB with the 15 synthetic members, and starts FastAPI serving both the API and the static `index.html` on a single port. Open `localhost:8000` — no Node, no second process, no CORS. The UI is one static file (vanilla JS); `uv` makes setup faster, and a CLI chat loop is the honest fallback if even the page is more than time allows.

**Hosted deploy — online, one place.** A single Render Web Service, deployed from a committed **`render.yaml`** Blueprint: FastAPI serves the API *and* the static page same-origin. Render's build step runs on separate compute that can't see the runtime filesystem, so `init_db` **and a seed-if-empty** run idempotently on app startup rather than at build time, followed by the ingest-path auto-scan — a fresh instance self-heals its schema, the 15 training members, *and* their observations (reusing the idempotent `ingest_dataset`, the same loader `/admin/reseed` calls, then `pipeline.scan_members`; findings are on the page from the first load, no manual scan). **As shipped this runs on the FREE plan — no persistent disk**, so `HEALTH_DB_PATH` is left unset and SQLite lands on the container filesystem at the code's default (`backend/data/health.db`), wiped whenever Render cold-starts the idle-spun-down instance and re-seeded from `training_data`: `POST /members` uploads, `feedback`, and promoted `prompt_versions` do not survive a spin-down — a documented trade-off, not a failure, and notably *not* an untested branch, since unset-`HEALTH_DB_PATH` + seed-if-empty is exactly what a local `make run` exercises. The **durable upgrade** is a one-block `render.yaml` change and no code change: the paid `starter` plan with a 1 GB disk at `/data` and `HEALTH_DB_PATH=/data/health.db`, after which live ingest survives deploys (the startup seed-if-empty self-heals a *fresh* disk and leaves a non-empty one alone). That would persist everything in the DB (uploaded **members** + feedback + **promoted `prompt_versions`**); the dataset *folders* `POST /members/upload` writes (the kept `eval_set.jsonl`/`lab_panels.csv`) live under the datasets root, ephemeral unless `HEALTH_DATA_ROOT` is also pointed at the disk — and a relocated empty root needs the shipped `training_data` copied in once, since the startup seed reads it from there (`render.yaml` documents this). Two consequences belong to the durable tier specifically and are **moot on free**, which rebuilds from `training_data` every cold start: a promoted learned prompt is a *frozen* snapshot with the composer's emergency-contact copy interpolated into it, so a `config.py` emergency-number change (a safety change) requires `POST /reset` or `/admin/reseed` afterward to drop any stale promoted `vN` (the deterministic `seek_care`/`crisis` templates read the numbers live and never go stale — only the composer prompt does); and an `analysis.py`/`config.py` change deployed over a durable DB needs a manual Scan to refresh persisted observations, since nothing re-derives them on deploy. One repo, one service, one URL; the local run stays primary — a clean local run takes priority over a hosted deploy.

**One engine, both places — SQLite.** The same database runs locally and in production; the dev/prod engine split is avoided deliberately. A split would put an untested gap on exactly the seams that differ between engines — type strictness, date handling, the idempotent-insert form, write-concurrency semantics — and, worse, would let the eval harness (§8) certify a stack the product never ships. SQLite also fits the design on its own terms: per-member data is tiny and recomputed on read, so the store is a file, not a server. The cost of durability is explicit and was **declined for the hosted demo**: a Render persistent disk is a paid tier (~$8/mo), and the free tier has no disk and spins down when idle, which wipes the file — so `POST /members` uploads and `feedback` do not survive a cold start, and the demo trades live-state durability for a free, shareable URL. Nothing about the design depends on that choice: the disk is three lines of `render.yaml` (§15), and the startup seed-if-empty that makes the ephemeral tier safe is the same self-heal a fresh disk would need. (A disk also pins the service to one instance with no zero-downtime deploy; the free plan likewise does not scale horizontally, so the single-host scope of the cross-process locks holds on either tier.) The hosted instance runs **one uvicorn worker** to fit the free tier's 512 MB, while `make serve` runs **two** against the same file — and the concurrency story is written for the multi-worker case, so the deployed configuration is the strictly easier one: the deterministic core is pure and every concurrency-sensitive write is an atomic idempotent operation on a UNIQUE key (`INSERT OR IGNORE`), so fire-once, idempotency, and floor enforcement hold under any worker count; the few check-then-act sequences that span statements — startup init+seed, `/learn`, `/reset`, `/admin/reseed`, and the dataset ingests (`/members/upload` · `POST /members`) — are serialized by cross-process file locks (`db.process_lock`, §11), each member scan rides one `BEGIN IMMEDIATE` transaction (§11), and `db.connect` retries the one-time delete→WAL switch that two racing first-boots hit. Scaling wider is a bounded seam (§11), not a rework — and the deploy is single-instance on either tier (free doesn't scale out; a paid disk pins it), so cross-host coordination never arises. Portability is kept as a cheap seam rather than paid for now: `db.py` is the only module touching the store, its queries are hand-written, parameterized, and kept to standard SQL forms, and the one dialect-specific operation — the idempotent escalation insert (`INSERT OR IGNORE`) — maps directly to Postgres `ON CONFLICT DO NOTHING`. Moving to Postgres if scale ever demanded it is therefore a connection-string change and one insert, not a rewrite.
