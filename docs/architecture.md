...
flowchart TD
  ...
  RT -- none --> C[Compose · LLM<br/>temp 0 · drafts prose + cited markers<br/>code attaches evidence]:::gen
...
flowchart TB
  ...
  GATE -->|route none → compose| ASST[assistant LLM · compose<br/>temp 0 · drafts prose + cited markers<br/>code attaches evidence]:::gen
...
| `POST /members/upload` | Upload a **whole dataset bundle** — the control panel's **Upload bundle** button. Accepts a `.zip` of a `training_data`-shaped folder (one CSV = lab panels; the members JSON + an optional eval JSON, **discovered by type/content, NOT filename** — the hold-out ships identical schemas under possibly-different names, distinguished by record shape: members records carry `profile`/`panels`, eval records don't) or a bare members-shaped `.json`. Effect: (1) **persists the entire bundle** as a new `<data-root>/<name>/` dataset folder, each file written under its **canonical** name (so the eval adapter's name-based read keeps working; name = optional `name` field, else the file's stem; **never overwrites** — a collision is 409), so the kept `eval_set.jsonl` is usable later via `DATASET=<name> make eval`; (2) ingests its members **additively** (upsert per id — added on top of existing members, no reseed; `member_id` is opaque, any ID space); (3) **auto-scans** the new members so their Observations match their live Trajectory at once. Doubles as the format check (bad zip / unresolvable bundle / malformed member → 422, never 500); classified before any side effect, shape validated up front (no partial load; the `ingest_members` semantic-partial-set caveat applies). | `multipart file [+ name]` → `{dataset, members, results, ranges, member_ids, files, scanned}` |
...
| `POST /admin/reseed` | **Factory reset** (demo hygiene; distinct from `/reset`): truncate all tables, restore baseline `prompt_version` v0, re-ingest the `training_data` bundle only — back to the initial 15-member state, uploaded holdouts dropped. An operator op, not a member feature. | → `{reseeded}` |
...
│   │   └── __main__.py          # CLI entry (make eval) → eval/reports/
