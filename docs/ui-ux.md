...
**Upload bundle** is the holdout path — how an unseen dataset is brought in at runtime. The holdout is data we've never seen, so it can't be a pre-baked option: the button reads a **`.zip` of a `training_data`-shaped folder** (a lab-panels CSV + a members JSON + an optional eval JSON) or a bare members JSON, `POST`s it as multipart to `/members/upload`. Files are matched by **type/content, not name** — the hold-out may ship identical schemas under different filenames (`panel.csv`, `roster.json`, `cases.json`), so the members file is found by its `profile`/`panels` records, not by being called `members.json` — and the server validates every bundle against the Pydantic models, so a bad zip or malformed bundle returns a clear error in the same readout: the upload *is* the format check. The server then (1) **saves the whole bundle** as a new dataset folder under `backend/data/` (named after the zip, so the kept `eval_set.jsonl` is usable later via `DATASET=<name> make eval`; a name collision is a clear 409, never an overwrite), (2) ingests the members **additively** — added on top of the existing data, no reseed — and (3) **auto-scans** the new members so their Observations are populated at once (reseed deliberately does not). On success the header **member-picker** gains the uploaded members and switches to the last one, and the surface repopulates for the new data. So on the deployment, an unseen dataset can be loaded and demoed immediately — no redeploy, no curl — and the picker selects among the 15 seeded members and anything uploaded. The single-bundle `POST /members` is documented in the README as a `curl` for a scriptable equivalent.
...
The right panel's second tab, **Trajectory**, is the member-facing counterpart to the operator's trajectory readout: one rough per-marker sparkline (readings · Theil–Sen line · flagged points; no axes/legend/tooltips) over `GET /members/{id}/trajectory`, ordered flagged-markers-first to mirror the Observations ranking. It is **presentation only** — like the operator readout, it shows the raw per-marker series for human eyes, while the **LLM still consumes only the collapsed `TrajectoryAnalysis` verdict, never this series** (architecture §4/§207). The tab lazy-loads on first open and caches per member (invalidated on member switch and on a re-scan).

**Both right-panel tabs gate on the scan together.** Until a scan has produced observations for the member, *both* tabs show the same `"No results to show yet — run a scan to refresh."` state — never the misleading split of a full Trajectory beside an empty Observations. Since any flagged marker raises an observation when scanned, this also guarantees the Trajectory tab never asserts a flag the Observations tab doesn't. The wording is deliberately *not* "Not analyzed yet" — that would be false for a scanned-clean member (a calm member's scan correctly writes zero observations) and would contradict an active care-team flag. One residual limit of using "has observations" as the analyzed signal: a *healthy* member who raises nothing (a negative control like C02/C08) still can't open the Trajectory tab even after a scan, because a zero-finding scan leaves no DB trace — distinguishing scanned-healthy from never-scanned would need a persisted scan marker (a deliberate non-goal of the lean schema). *(Build note, Phase 8.)*
