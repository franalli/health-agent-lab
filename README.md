...
The demo's operator panel (left rail, kept out of the member experience) drives every endpoint with one click, and **Upload bundle** brings a held-out dataset in at runtime — no redeploy. It accepts a **`.zip` of a `training_data`-shaped folder** (a lab-panels CSV + a members JSON + an optional eval JSON) or a bare members JSON, and (1) **persists the whole bundle** as a new dataset folder under `backend/data/` — files matched by **type/content, not filename**, so the hold-out's names may differ (the members file is the JSON whose records carry `profile`/`panels`) and are normalized to canonical names on disk, keeping the `eval_set.jsonl` usable later via `DATASET=<name> make eval`; (2) ingests every member **additively** (added on top of the seeded data, no reseed; `member_id` is opaque); and (3) **auto-scans** the new members (`POST /members/upload`). A malformed/unresolvable file returns a clear error in the operator readout, so the upload doubles as the format check. The same ingest runs as a CLI (`uv run python -m preprocessing.ingest <bundle>`); `POST /members` is the single-bundle live equivalent.
