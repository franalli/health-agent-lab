...
`ls` shows the tree; the file-by-file map is `architecture.md` §14 (the one canonical copy — don't duplicate it here). What `ls` won't tell you is intent, and that's in the invariants above: `health_intelligence/` is the serving library (pure, imports no web framework), `preprocessing/ingest.py` is the one firewall, `eval/` is the harness, and `api.py` is thin routes serving the single static `frontend/index.html`. *(The build is at Phase 6: `frontend/index.html` ships as the consumer surface — Mode-1 chips + Mode-2 ask, proactive observations, loud-once-then-ambient escalation, and an operator control panel built from a route registry — and `api.py` serves it alongside the full §13 member routes (`GET`/`POST`/`DELETE /members` landed here, backed by `db.list_member_summaries` / `db.delete_member`). Forward increments remain: the eval LLM judge (semantic grounding, tone — `make eval` otherwise live), Phase 7 self-improvement (`/learn`·`/feedback`·`/reset`, which the control panel's Learning group drops in when scaffolded), and Phase 8 deploy. The invariants describe the target design throughout.)*
