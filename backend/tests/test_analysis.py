...
def _results(marker, unit, values, dates=None):
    ...
    return [
        ...
        for i, (dt, v) in enumerate(zip(dates, values), 1)
    ]


def _range(marker, sex="any", unit="", ref_low=None, ref_high=None, panic_low=None, panic_high=None):
    return ReferenceRange(
        marker=marker, sex=sex, unit=unit, ref_low=ref_low, ref_high=ref_high,
        panic_low=panic_low, panic_high=panic_high, config_version="v0",
    )
...
def test_seeded_slope_is_detected_and_its_rate_recovered():
    ...
    out = analysis.analyze(
        _member(), _results("eGFR", "mL/min/1.73m2", values, dates),
        [_range("eGFR", ref_low=90.0)], age=50, cfg=CFG, data_version="d1",
    )
    ...
    assert egfr.clinical_change.exceeds_rcv is True      # 100 -> ~78 (~-22%) clears eGFR RCV
...
def test_n3_is_too_short_to_call_a_trend_but_flags_still_compute():
    ...
    out = analysis.analyze(
        _member("C12"), _results("eGFR", "mL/min/1.73m2", [75.0, 68.0, 61.0]),
        [_range("eGFR", ref_low=90.0)], age=58, cfg=CFG, data_version="d1",
    )
    egfr = _marker(out, "eGFR")
...
def test_in_range_noise_reads_as_noise_not_a_trend():
    ...
    out = analysis.analyze(
        _member("C08"), _results("eGFR", "mL/min/1.73m2", [95.0, 92.0, 99.0, 91.0, 99.0, 93.0, 95.0]),
        [_range("eGFR", ref_low=90.0)], age=44, cfg=CFG, data_version="d1",
    )
    egfr = _marker(out, "eGFR")
    ...
    assert egfr.trend.direction == "flat"       # Theil-Sen CI spans zero -> too noisy to sign
...
def test_beneficial_trend_does_not_raise_severity():
    ...
    out = analysis.analyze(
        _member("C03"), _results("LDL cholesterol", "mg/dL", [168.0, 150.0, 138.0, 125.0, 118.0]),
        [_range("LDL cholesterol", ref_high=100.0)], age=52, cfg=CFG, data_version="d1",
    )
    ldl = _marker(out, "LDL cholesterol")
    assert ldl.trend.significant is True
    assert ldl.trend.direction == "decreasing"
    assert ldl.severity == "notable"           # above range, but the beneficial trend adds nothing
    assert out.overall_floor == "none"
...
def test_panic_value_forces_urgent_and_potassium_skips_rcv():
    out = analysis.analyze(
        _member("C07"), _results("Potassium", "mmol/L", [4.2, 4.3, 4.4, 4.5, 6.1]),
        [_range("Potassium", ref_low=3.5, ref_high=5.1, panic_high=6.0)],
        age=60, cfg=CFG, data_version="d1",
    )
    k = _marker(out, "Potassium")
    assert "panic_high" in k.flags
    assert k.severity == "urgent"
    assert out.overall_floor == "urgent"
    assert k.clinical_change.exceeds_rcv is None   # Potassium has no CVa/CVi -> RCV skip-path, not silent
...
def test_panic_low_pins_urgent():
    ...
    out = analysis.analyze(
        _member("C07"), _results("Potassium", "mmol/L", [4.5, 4.2, 3.6, 3.0, 2.5]),
        [_range("Potassium", ref_low=3.5, ref_high=5.1, panic_low=2.8)],
        age=55, cfg=CFG, data_version="d1",
    )
    k = _marker(out, "Potassium")
...
def test_increasing_adverse_trend_escalates_end_to_end():
    ...
    out = analysis.analyze(
        _member(), _results("HbA1c", "%", [5.6, 5.8, 6.0, 6.2, 6.4]),
        [_range("HbA1c", ref_high=5.7)], age=50, cfg=CFG, data_version="d1",
    )
...
def test_vitamin_d_band_cross_with_adverse_decline_escalates():
    ...
    out = analysis.analyze(
        _member("C13"), _results("Vitamin D (25-OH)", "ng/mL", [28.0, 22.0, 18.0, 14.0, 12.0]),
        [_range("Vitamin D (25-OH)", ref_low=20.0)], age=39, cfg=CFG, data_version="d1",
    )
...
def test_tied_series_yields_a_valid_pvalue_without_crashing():
    out = analysis.analyze(
        _member(), _results("HbA1c", "%", [5.5, 5.5, 5.6, 5.8, 6.0]),
        [_range("HbA1c", ref_high=5.7)], age=46, cfg=CFG, data_version="d1",
    )
    hba1c = _marker(out, "HbA1c")
...
def test_exact_pvalue_engines_agree_at_their_boundary():
    ...
    assert analysis._exact_p_enum(values, s) == pytest.approx(analysis._exact_p_inversions(len(values), s))
...
def test_fdr_keeps_significant_candidates_and_drops_sub_alpha_trends():
    ...
    out = analysis.analyze(
        ...
        age=50, cfg=CFG, data_version="d1",
    )
    assert _marker(out, "eGFR").trend.significant is True
    assert _marker(out, "CRP").trend.significant is False   # p >= alpha -> never a candidate
...
def test_bh_rejection_branch_activates_only_when_q_below_alpha():
    ...
    inert = AnalysisConfig(version="t", stats=StatConfig(alpha=0.05, fdr_q=0.10), markers={})
    assert analysis._fdr(trends, inert) == {"a": True, "b": True, "c": True}  # vacuous (q > alpha)

    active = AnalysisConfig(version="t", stats=StatConfig(alpha=0.05, fdr_q=0.01), markers={})
...
def test_marker_without_a_range_gets_a_typed_no_reference_note():
    out = analysis.analyze(
        _member(), _results("ALT", "U/L", [20.0, 22.0]), ranges=[], age=50, cfg=CFG, data_version="d1",
    )
    alt = _marker(out, "ALT")
...
def test_range_selection_prefers_sex_then_falls_back_to_any():
    ...
    assert analysis._range_for("Hemoglobin", "unknown", 30, sexed) is None  # no 'any' to fall back to
...
def test_output_is_deterministic():
    args = (
        _member(), _results("eGFR", "mL/min/1.73m2", [100.0 - i for i in range(6)]),
...
def test_analysis_module_is_pure():
    ...
    banned_top = {"sqlite3", "requests", "httpx", "urllib", "socket", "http",
                  "os", "sys", "time", "random", "secrets", "anthropic", "asyncio"}
    assert not (top_level & banned_top), f"impure import: {top_level & banned_top}"
    banned_full = {"health_intelligence.db", "health_intelligence.llm",
                   "health_intelligence.gate", "health_intelligence.pipeline"}
    assert not (full & banned_full), f"core imports a downstream layer: {full & banned_full}"
    for token in (".now(", ".today(", "time.time", "monotonic(", "perf_counter", "urandom"):
...
def test_no_rcv_marker_surfaces_trend_at_notable_not_attention():
    ...
    out = analysis.analyze(
        _member(), _results("bmi", "kg/m2", [21.0, 21.8, 22.5, 23.3, 24.0]),
        [_range("bmi", ref_low=18.5, ref_high=25.0)], age=40, cfg=CFG, data_version="d1",
    )
    bmi = _marker(out, "bmi")
...
def test_systolic_bp_is_rcv_gated_and_can_escalate():
    ...
    out = analysis.analyze(
        _member(), _results("systolic_bp", "mmHg", [120.0, 126.0, 132.0, 138.0, 145.0]),
        [_range("systolic_bp", ref_low=90.0, ref_high=120.0)], age=58, cfg=CFG, data_version="d1",
    )
...
def test_normal_approx_path_detects_a_tied_high_n_trend():
    ...
    values = [10.0, 11.0, 11.0, 13.0, 14.0, 15.0, 16.0, 17.0, 18.0, 19.0, 20.0]  # n=11, a tie -> _normal_p
    out = analysis.analyze(
        _member(), _results("CRP", "mg/L", values), [_range("CRP", ref_high=3.0)],
        age=50, cfg=CFG, data_version="d1",
    )
    crp = _marker(out, "CRP")
    assert crp.trend.direction == "increasing"
    assert crp.trend.p_value < 0.05          # the normal approximation did not invert a clear trend
...
def test_single_reading_still_computes_flags():
    ...
    out = analysis.analyze(
        _member(), _results("Potassium", "mmol/L", [6.5]),
        [_range("Potassium", ref_low=3.5, ref_high=5.1, panic_high=6.0)],
        age=50, cfg=CFG, data_version="d1",
    )
    k = _marker(out, "Potassium")
...
def test_analyze_selects_sex_specific_range_through_the_pipeline():
    ...
    fem = analysis.analyze(_member("F1", sex="female"), _results("Hemoglobin", "g/dL", [12.6, 12.5]),
                           ranges, age=30, cfg=CFG, data_version="d1")
    assert "below_range" not in _marker(fem, "Hemoglobin").flags   # female band applied
    male = analysis.analyze(_member("M1", sex="male"), _results("Hemoglobin", "g/dL", [12.6, 12.5]),
                            ranges, age=30, cfg=CFG, data_version="d1")
