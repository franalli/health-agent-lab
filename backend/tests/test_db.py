...
def _bundle(member_id, panels, *, sex="male", age=50, conditions=None, medications=None,
            family_history=None, lifestyle=None, notes=None):
    return MemberBundle.model_validate({
        "member_id": member_id,
        "profile": {
            "member_id": member_id, "age": age, "sex": sex,
            "conditions": conditions or [], "medications": medications or [],
            "family_history": family_history or [], "lifestyle": lifestyle or {},
        },
        "panels": panels,
        "notes": notes or [],
    })
...
def _k_panels(values):
    ...
    return [_panel(f"M-P{i+1}", dates[i], [_r("Potassium", v, "mmol/L", "3.5-5.1")])
...
def test_emit_escalation_is_idempotent():
    ...
    created1 = db.emit_escalation(con, member_id="C07", kind="data_finding", dedup_key=key,
                                  level="urgent", trigger_reason="K+ 6.1")
    created2 = db.emit_escalation(con, member_id="C07", kind="data_finding", dedup_key=key,
                                  level="urgent", trigger_reason="K+ 6.1")
    assert created1 is True and created2 is False
    n = con.execute("SELECT COUNT(*) FROM escalations WHERE dedup_key = ?", (key,)).fetchone()[0]
    assert n == 1
...
def test_member_round_trips_through_db():
    ...
    bundle = _bundle(
        "M3",
        [_panel("M3-P1", "2024-01-01",
                [_r("Hemoglobin", 12.5, "g/dL", "13.5-17.5 (male) / 12.0-15.5 (female)")])],
        sex="female", age=29,
        conditions=["iron-deficiency anemia"], medications=["ferrous sulfate"],
        family_history=["mother: hypothyroidism"], lifestyle={"diet": "vegetarian"},
    ...
    assert p.conditions == ["iron-deficiency anemia"] and p.medications == ["ferrous sulfate"]
    assert p.family_history == ["mother: hypothyroidism"] and p.lifestyle == {"diet": "vegetarian"}

    notes = db.get_notes(con, "M3")
    assert len(notes) == 1 and notes[0].source == "in-app" and notes[0].text == "feeling tired"
...
def test_vitals_folded_as_markers_with_config_units():
    ...
    ranges = {(r.marker, r.sex): r for r in db.get_ranges(con)}
    assert (ranges[("bmi", "any")].ref_low, ranges[("bmi", "any")].ref_high) == (18.5, 25.0)
...
def test_reingest_replaces_member_and_bumps_data_version():
    con = _con()
    panels = [_panel(f"M4-P{i}", d, [_r("HbA1c", v, "%", "<5.7")])
              for i, (d, v) in enumerate(
                  [("2024-01-01", 5.3), ("2024-07-01", 5.4), ("2025-01-01", 5.3), ("2025-07-01", 5.4)],
                  start=1)]
    ...
    changed = [_panel(f"M4-P{i}", d, [_r("HbA1c", v, "%", "<5.7")])
               for i, (d, v) in enumerate(
                   [("2024-01-01", 5.3), ("2024-07-01", 5.4), ("2025-01-01", 5.3), ("2025-07-01", 6.4)],
                   start=1)]
...
def test_resolve_overrides_returns_new_lists_without_mutating():
    ...
    con.commit()
    results = [LabResult(marker="HbA1c", value=5.3, unit="%", panel_id="p1", panel_date="2024-01-01")]
    ranges = [ReferenceRange(marker="HbA1c", sex="any", unit="%", ref_high=5.7,
...
def test_all_supplied_members_analyze_over_the_db():
    ...
    for mid in db.list_members(con):
        ...
        floors[mid] = analyze(member, results, ranges, age, ANALYSIS_CONFIG, data_version=dv).overall_floor
...
def test_sex_split_panic_reaches_unknown_sex_member():
    ...
    con = _con()
    panels = [_panel(f"U-P{i}", d,
                     [_r("Hemoglobin", v, "g/dL", "13.5-17.5 (male) / 12.0-15.5 (female)")])
              for i, (d, v) in enumerate(
                  [("2024-01-01", 11.0), ("2024-07-01", 9.5), ("2025-01-01", 8.0), ("2025-07-01", 6.5)],
                  start=1)]
    ...
    assert ranges[("Hemoglobin", "any")].ref_low is None and ranges[("Hemoglobin", "any")].ref_high is None
...
def test_divergent_reference_range_across_members_is_rejected():
    ...
    con = _con()
    ingest_bundle(con, _bundle("A1", [_panel("A-P1", "2024-01-01", [_r("HbA1c", 5.3, "%", "<5.7")])]))
    with pytest.raises(ValueError, match="diverges from the stored definition"):
        ingest_bundle(con, _bundle("B1", [_panel("B-P1", "2024-01-01", [_r("HbA1c", 5.3, "%", "<6.0")])],
                                   sex="female"))
...
def test_load_for_analysis_matches_in_memory_fixture():
    con = _con()
    series = [("2024-01-05", 96.0), ("2024-07-05", 95.0), ("2025-01-05", 97.0), ("2025-07-05", 96.0)]
    panels = [_panel(f"M2-P{i}", d, [_r("eGFR", v, "mL/min/1.73m2", ">=90")])
    ...
    db_analysis = analyze(member, results, ranges, age, ANALYSIS_CONFIG, data_version=dv)

    # the same eGFR inputs assembled directly in memory must yield the same per-marker verdict
    mem_results = [LabResult(marker="eGFR", value=v, unit="mL/min/1.73m2",
                             panel_id=f"M2-P{i}", panel_date=d)
                   for i, (d, v) in enumerate(series, start=1)]
    mem_ranges = [ReferenceRange(marker="eGFR", sex="any", unit="mL/min/1.73m2",
                                 ref_low=90.0, ref_high=None, config_version=CONFIG_VERSION)]
    mem_analysis = analyze(member, mem_results, mem_ranges, 50, ANALYSIS_CONFIG, data_version=dv)
    ...
    assert db_analysis.overall_floor == "none"           # eGFR + default vitals all in-range/stable
