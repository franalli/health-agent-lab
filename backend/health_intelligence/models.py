...
class Observation(BaseModel):
    ...
    p=0.017, tau=1.00, n=5, Theil-Sen slope 0.03/day; clears reference-change value" — the full
    ``_trend_stat``, rate included, so the queue can triage on HOW FAST), PERSISTED and surfaced only on
    the operator console / clinician queue; ``member_explanation`` is the member-facing plain-language "what this means for
